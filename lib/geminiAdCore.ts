  ...
  for (const model of candidateModels) {
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (rawText) {
          return JSON.parse(rawText) as AdConceptData;
        }
      } catch (err: any) {
        lastError = err;
        ...
        if (isTemporarySpike) {
          console.warn(`Model ${model} spike (503/429). Falling back to next model...`);
          break;
        }
...
export async function generateAdCore(params: {
  apiKey: string;
  text?: string;
  ...
  const { apiKey, text, fileData, targetLanguage, angleStyle, tone, variationSeed } = params;
...
export async function refineAdCore(params: {
  apiKey: string;
  currentAd: AdConceptData;
  instruction: string;
  targetLanguage?: string;
}): Promise<AdConceptData> {
  const { apiKey, currentAd, instruction, targetLanguage } = params;
