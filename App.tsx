...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const generated = await requestGenerateAd({
        text: inputText,
        fileData,
        targetLanguage,
        angleStyle,
  ...
  const handleRefine = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await requestRefineAd({
        currentAd: adData,
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans antialiased">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        {/* Error Banner */}
        {errorMessage && (
          <div className="bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-xl text-xs sm:text-sm flex items-start justify-between gap-3 shadow-xs">
            <div className="flex items-start gap-2.5">
              <AlertCircle className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
              <div>
                <span className="font-bold block mb-0.5">
                  {lang === "bn" ? "সমস্যা হয়েছে:" : "Error Occurred:"}
                </span>
                <p className="leading-relaxed text-rose-900">{errorMessage}</p>
              </div>
            </div>
            <button
              onClick={() => setErrorMessage(null)}
              className="text-rose-600 hover:text-rose-900 font-bold px-2 py-1 text-xs shrink-0 cursor-pointer"
            >
